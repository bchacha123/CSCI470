...
public class TileCanvas extends JPanel implements MouseListener
{
	...
	{
		for (int i = 0; i < DesignLayout.imageA.length; i++) 
		{
			DesignLayout.imageA[i] = (Image) Toolkit.getDefaultToolkit().getImage(DesignLayout.IMAGESTRINGNAMEA[i]);
		}
	}// End patchArray
	...
	public void mouseClicked(MouseEvent e)
	{
		if(selectedTile >= 0)
		{
			...
			if(x >= beginX && x <= beginX + gwidth && y >= beginY && y <= beginY + gheight) 
			{
                ...
                gif2dArray[xin][yin] = DesignLayout.imageA[selectedTile]; 
